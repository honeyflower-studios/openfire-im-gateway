...
public class XMPPListener implements MessageListener, ConnectionListener, ChatManagerListener, PacketListener, RosterListener {
     ...
     */
    public XMPPListener(/*XMPPSession session,*/ ActiveConnection connection) {
    ...
    public void processMessage(final Chat chat, final org.jivesoftware.smack.packet.Message message) {
        connection.execute(new ConnectionCommand() {
            ...
            public void execute(XMPPSession session, XMPPConnection conn, XMPPListener listener) {
                Log.debug("Received "+ session.getTransport().getType().name()+" message: "+message.toXML());
                try {
                    final BaseTransport<XMPPBuddy> transport = session.getTransport();
                    final JID legacyJID = transport.convertIDToJID(message.getFrom());
                    final JID localJID = session.getJID();
...
//                    }
    ...
    public void connectionClosed() {
        connection.disconnected(null, null, false);
        //getSession().sessionDisconnectedNoReconnect(null);
    }

    public void connectionClosedOnError(Exception exception) {
        connection.disconnected(
                LocaleUtils.getLocalizedString("gateway.xmpp.connectionclosed", "kraken"), 
...
//        getSession().sessionDisconnected(LocaleUtils.getLocalizedString("gateway.xmpp.connectionclosed", "kraken"));
            ...
            public void execute(XMPPSession session, XMPPConnection conn, XMPPListener listener) {
                if (packet instanceof GoogleMailBoxPacket) {
                    if (JiveGlobals.getBooleanProperty("plugin.gateway.gtalk.mailnotifications", true)) {
                        ...
                        setLastGMailThreadDate(mbp.getResultTime());
                        Integer newMailCount = 0;
                        String mailList = "";
                        for (GoogleMailThread mail : mbp.getMailThreads()) {
                            newMailCount++;
                            if (getLastGMailThreadId() == null || mail.getThreadId() > getLastGMailThreadId()) {
                                setLastGMailThreadId(mail.getThreadId());
                            }
                            String senderList = "";
                        ...
                        if (newMailCount > 0) {
                            session.getTransport().sendMessage(
                                    session.getJID(),
                                    session.getTransport().getJID(),
                    ...
                    if (gnme != null) {
                        ...
                        conn.sendPacket(new IQWithPacketExtension(new GoogleMailNotifyExtension()));
                    }
                }
        ...
        connection.execute(new ConnectionCommand() {
            ...
            public void execute(XMPPSession session, XMPPConnection conn, XMPPListener listener) {
                for (String addr : addresses) {
                    RosterEntry entry = conn.getRoster().getEntry(addr);
                    session.getBuddyManager().storeBuddy(new XMPPBuddy(session.getBuddyManager(), entry.getUser(), entry.getName(), entry.getGroups(), entry));
                    ...
                    if (!TransportType.facebook.equals(session.getTransport().getType())) {
                        //ProbePacket probe = new ProbePacket(session.getJID()+"/"+session.xmppResource, entry.getUser());
    ...
    public void entriesDeleted(final Collection<String> addresses) {
        connection.execute(new ConnectionCommand() {
            ...
            public void execute(XMPPSession session, XMPPConnection conn, XMPPListener listener) {
                for (String addr : addresses) {
                    session.getBuddyManager().removeBuddy(addr);
                }
            }
        });
    }
